...
let addToCart = (e) =>{
    ...
    if(cartItems.some(element => element.id == i)){
        smoothies.forEach(element => {if(element.id == i)alert(`${element.smoothie} has already been added to cart`)})
    }else{
        smoothies.forEach((element) => {if(element.id == i)cartItems.push(element)});
